...
| STW（Stop-The-World） | GC 暂停 JS 执行的时刻，直接计入请求延迟 | 第 2 章 |
| internalBinding | 内置 JS 模块获取 C++ 能力的唯一通道，用户代码不可见 | 第 3 章 |
| BaseObject | JS 对象与 C++ 对象一对一双向绑定的地基类 | 第 3 章 |
| MakeCallback | C++ 世界调用 JS 回调的统一入口（"底层向 JS 喊话的话筒"） | 第 3 章 |
| FunctionTemplate | 把 C++ 函数铸成 JS 函数的 V8 模具；internalBinding 的导出皆出于此 | 第 3 章 |
| CallbackScope | MakeCallback 三段式之首：进场时切换异步上下文；TryCatch 兜住 JS 异常 | 第 3 章 |
| 事件循环 | libuv uv_run 的七阶段循环，Node 异步的心脏 | 第 4 章 |
| poll 阶段 | 循环的主腔：调用 epoll_wait 向内核收割就绪事件，空闲时在此阻塞 | 第 4 章 |
| 微任务 | nextTick 队列与 Promise 队列，在阶段间隙排空，可饿死循环 | 第 4 章 |
| uv_run 三模式 | DEFAULT 转到没活 / ONCE 转一圈可阻塞 / NOWAIT 转一圈不阻塞；Node 用 DEFAULT | 第 4 章 |
| epoll | Linux 的 I/O 多路复用机制：成本只与就绪事件数相关 | 第 5 章 |
| EAGAIN | 非阻塞 fd "暂时无数据"的返回码；边缘触发下读到它才停 | 第 5 章 |
| 线程池 | libuv 默认 4 线程，替主线程执行阻塞操作（文件/DNS/部分 crypto） | 第 5 章 |
| fd | 进程私有的整数号码牌，指向内核 struct file | 第 6 章 |
| inode | 内核中资源的本体；socket/pipe 的 inode 是内存匿名对象 | 第 6 章 |
| SCM_RIGHTS | 通过 Unix 域套接字把 fd 传给另一个进程的机制 | 第 6 章 |
| dup2 | 重定向的本质：fork 后 exec 前改写 fd 表项的指向 | 第 6 章 |
| NODE_CHANNEL_FD | fork 时用环境变量向子进程交接 IPC 通道的 fd 号码 | 第 6 章 |
| EMFILE | fd 表耗尽错误；fd 泄漏的雪崩终点 | 第 6 章 |
| BackingStore | ArrayBuffer 持有的堆外裸内存；Buffer 零拷贝的物理基础 | 第 7 章 |
| allocUnsafe | 不清零的快速分配；残留数据是真实安全风险 | 第 7 章 |
| highWaterMark | 流内部缓冲的水位线；"建议停"而非硬上限 | 第 8 章 |
...
| AsyncWrap | C++ 侧异步资源基类，签发 asyncId 族谱的出生证明 | 第 15 章 |
| async_hooks | 异步族谱的观测 API（init/before/after/destroy）；代价不低，留给诊断工具 | 第 15 章 |
| AsyncContextFrame | 把上下文帧挂进 Promise 延续体的第二代实现，免回溯、默认启用 | 第 15 章 |
...
| "事件驱动 = 异步" | emit 是同步 for 循环；异步的是事件的触发时机 | 第 9 章 |
| "所有异步 I/O 都走事件通知" | 只有网络走 epoll；文件/dns.lookup 走线程池模拟 | 第 5 章 |
| "setTimeout(fn,0) 先于 setImmediate" | 主模块顶层顺序不定；I/O 回调内 setImmediate 必先 | 第 4 章 |
| "fd 就是文件/连接本身" | fd 只是号码牌；本体是内核 inode | 第 6 章 |
| "IPC 是进程间发消息" | 本质是多个 fd 指向同一个内核对象 | 第 6 章 |
| "重定向是程序改了输出" | 程序零感知：shell 在 fork 后 exec 前用 dup2 换掉 fd 表指向 | 第 6 章 |
| "Buffer.length 是字符数" | 是字节数；多字节字符两者不等 | 第 7 章 |
| "highWaterMark 是缓冲上限" | 只是建议水位；无视 write 返回 false 照样 OOM | 第 8 章 |
...
| "process.env 是启动时的一份拷贝" | 是进程环境表的实时代理，读写直达 getenv/setenv | 第 12 章 |
| "process.exit() 是正常退出方式" | 是拉闸；会丢异步缓冲。正道是撤掉活跃句柄 | 第 12 章 |
| "Worker 之间共享变量" | 堆隔离；postMessage 是克隆，共享只有 SharedArrayBuffer | 第 14 章 |
| "AsyncLocalStorage 能自动跨线程传播" | store 的传播半径是一个 Environment；跨 Worker/子进程需随消息携带 + 对侧重新 run | 第 15 章 |
| "async_hooks 适合业务代码日常使用" | 它是诊断工具的地基：插桩开销与内存代价都明显，应用层用 AsyncLocalStorage | 第 15 章 |
...
| vm 模块 | 第 11 章的 Realm/Context 机制 |
...
[← 终章：万物归一](./ch16-epilogue.md) | [返回目录](./README.md)
