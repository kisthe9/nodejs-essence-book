...
```
 启动    [12]装配线 → [11]三层容器 → [3]require+穿门 → [6]listen-fd → [4]循环入睡
                                                                      │
 连接    内核握手 → [4/5]epoll 醒 → accept 得 conn-fd[6] → [9]emit('connection')
                                                                      │
 请求    字节进内核缓冲 → [5]循环read至EAGAIN → [7]零拷贝入Buffer → [9]emit('request')
                                                                      │
 处理    [5]readFile 走线程池，主线程继续接客 → 完工唤醒 → [3]MakeCallback → 你的cb
         （[15] 因果链全程随行：跨线程池、跨事件循环，上下文不丢）
                                                                      │
 响应    res.end → [8]背压控流 → [6]write(conn-fd) 多态分发 → 网卡 → 浏览器
                                                                      │
 退场    SIGTERM → [12]server.close 撤句柄 → 循环自然停 → 善终
（多核： [14]以上全部 × N 份容器，cluster 分发连接；跨容器靠 [15] 的"携带+重 run"接续）
```
