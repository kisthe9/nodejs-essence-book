...
`MakeCallback` 这个名字值得记住——它就是"底层世界向 JS 世界喊话"的话筒。第 3 章讲事件循环时，所有 I/O 事件回到 JS，走的都是它；2.5 节会拆开它的完整动作。
...
这套生死协议与第 3 章"活跃句柄让进程不退出"是一体两面：**句柄活着 ↔ JS 壳被保住 ↔ 事件循环有活干**——资源、对象、循环，三者的寿命绑在同一根绳上。
...
先看门出口处的铸造车间。`internalBinding('tcp_wrap')` 返回对象上的每个方法，都出自同一套模具：C++ 侧把函数写成统一签名（收一个参数包，内含 this、实参列表与调用信息），经 V8 的 **FunctionTemplate** 铸成函数模板，再实例化挂到导出对象上。同理，`new TCP()` 的 JS 对象出自一个声明了"内部字段计数"的 ObjectTemplate——2.4 节那两个藏指针的槽位，就是铸模时预留的。
...
- **JS 侧**：`on('data', cb)` 把函数塞进 EventEmitter 的字典（第 8 章），`setTimeout` 把函数挂上 Timeout 对象——回调住进了 JS 对象；
 ...
 libuv 事件 → C++ 回调 → MakeCallback：
   ...
   ② 在 TryCatch 兜底中调用 JS 函数
      ——回调里抛异常不会炸穿 C++ 栈；异常被兜住，回调返回后
        转投 process 的 'uncaughtException'（第 8 章的快速失败由此而来）
   ③ 回调返回后，排空 nextTick/微任务队列（第 3 章的"阶段间隙"）
      ——然后才把控制权交还事件循环
```

第 3 章会看到：事件循环的每个阶段放行回调，走的都是这同一段动作。

穿门不是免费的——每次 JS↔C++ 往返都有 scope 进出与参数封送的固定开销。所以热路径上的设计都在**批量穿门**：一次 read 尽量收割、数据按 chunk 分发，而不是每字节往返一次。第 6 章 Buffer 的零拷贝设计，也是在给这道门减负。
...
```
 ① 封装（本章）        ② 调度（第 3 章）      ③ 喂数（第 5 章）       ④ 认出（第 12 章）
