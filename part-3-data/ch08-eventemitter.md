...
那"异步"从哪来？从**谁在什么时候调用 emit**。第 3 章的链路现在可以补全最后一环：
...
推论一：**监听器阻塞 = 全体阻塞**。emit 的 for 循环里某个监听器耗时 100ms，后面的监听器、emit 的调用方、乃至整个事件循环（第 3 章）都等着这 100ms。
...
工程铁律由此而来：**每个 socket、每个 stream、每个长期存活的 EventEmitter，必须挂 'error' 监听**。第 7 章推荐 pipeline 的原因之一，正是它替流水线上每个流接管了错误。至于 `process.on('uncaughtException')`——它是记录现场、优雅退出用的最后底线，不是"继续运行"的免死金牌（进程状态已不可信）。

## 8.4 谁在继承 EventEmitter

第 7 章说 Stream 继承 EventEmitter，其实继承者遍布整个运行时：
...
- **asyncId 因果链**：每个异步资源（第 2 章 AsyncWrap 的那个"身份证"）记着自己的 id 和"是谁创建了我"（triggerAsyncId），串成完整的异步族谱，`async_hooks` 可以观测它——这是诊断工具的地基；
...
这里只给出结论性的全貌。这套机制凭什么成立？它能不能进一步穿过第 11 章的线程与进程边界，把不同执行现场串回同一条主线？——完整推导留给第四部收尾的第 12 章。
...
[← 上一章：Stream](./ch07-stream.md) | [下一章：运行时容器 →](../part-4-container/ch09-runtime-container.md)
