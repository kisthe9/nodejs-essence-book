...
前几节回答了"有什么"（族谱、观测窗、ALS）；这一节回答"它如何发生作用"。把暗线上的前三次穿越摆在一起看——第 2 章封装时埋下护照、第 3 章瓣膜放行时验照、第 5 章进程接力时另带——每一次上下文生效，都是同一套机制的三幕。
...
资源躺在最小堆里、epoll 的监控清单里、线程池的队列里——上下文随它一起休眠：不占栈、不消耗，静静等待瓣膜放行。这就是 12.1 节"存到比栈活得久的地方"的具象：**上下文不住在栈上，住在资源上**。

### 第三幕·换入换出（执行时）

回调进场前，运行时用资源里休眠的上下文换掉当前现场；回调返回后，再换回来。第 2 章 MakeCallback 三段式里的 CallbackScope，干的就是这次切换：
...
1. **同步分发没有穿越。** 监听器在 `run` 内注册、却在 `run` 外被 `emit`——emit 是同步 for 循环（第 8 章），监听器看到的是 emit 现场的上下文，与注册现场无关。同步调用不经过三幕，自然无照。
2. **池化资源的护照签发日不对。** 连接池、任务队列的资源在启动时就创建完毕——捕获发生在"池诞生时"，不是"本次请求进来时"。回调执行时换入的是池的上下文，请求的 store 自然缺席。这正是 12.3.1 节 AsyncResource 存在的工程理由：给池里每个任务**补签一张当时的护照**。
