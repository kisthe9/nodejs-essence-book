# 第 15 章 异步上下文：穿越边界的因果链

> **本章问题**：第 9 章留下过一个困境——回调跨越异步边界后，"我在为谁工作"这条信息就断了。第 14 章又立起一道道容器边界：线程、进程，层层隔离。有没有一种机制，能让一条"因果链"穿过所有异步回调、甚至穿过线程与进程的边界，把散落各处的执行现场串回同一条主线？
...
但异步把栈炸掉了。第 4 章讲过，回调执行时，原来的栈早已退栈；第 9 章又补了一刀：emit 是同步 for 循环，监听器拿到的调用栈属于"数据到达"这个事件，**不属于任何请求**。栈没了，"来龙去脉"就没了——这就是第 9 章 9.5 节的困境。

关键推论：既然栈靠不住，Node.js 必须把"来龙去脉"从栈里搬出来，存到一个**比栈活得久的地方**。这个地方就是本章的主角——异步因果链。

## 15.2 asyncId：每个异步操作的出生证明
...
这套出生证明由谁签发？回到第 3 章的机制：几乎所有内置异步对象的 C++ 侧都继承同一个基类 **AsyncWrap**（`src/async_wrap.cc`），它在构造时申领 asyncId 与 triggerAsyncId，析构时登记销毁。而第 3 章那个"话筒" MakeCallback 每次拉起 JS 回调时，会先进入一个 CallbackScope，把"当前正在执行的 asyncId"切换成该回调对应的值——"**当前上下文"就这样在一次次 C++→JS 的穿越中被维护着**。

## 15.3 async_hooks：族谱的观测窗
...
### 15.3.1 AsyncResource：给自定义异步原语补一张出生证
...
## 15.4 AsyncLocalStorage：应用层的标准答案

第 9 章给过它的使用形态，现在看它凭什么成立：
...
**第二代：AsyncContextFrame（帧随延续体传播）。** 既然每个异步边界都要查一次，不如让上下文**自己跟着走**：Node 与 V8 协作，把"当前上下文帧"直接挂进 Promise 的延续体与异步边界（`lib/internal/async_context_frame.js`、`src/async_context_frame.cc`）——await 恢复、定时器到期、回调入场时，帧自动就位，无需回查。第 4 章的微任务队列、timers、nextTick 队列都各自保存并恢复帧（`lib/internal/timers.js`、`lib/internal/process/tasks_queues.js` 中可见），这正是"所有异步边界都认得上下文"的原因。新实现开销已低到可默认开启、用于生产——第 9 章那句"开销已低到可默认用于生产"，底层依据就在这里。

两套实现在 `lib/internal/async_local_storage/` 下并存，接口完全一致，应用代码无感。**把复杂度压进运行时、把简单露给应用**——这是 Node 内部模块的一贯做派（第 12 章 primordials 也是同一立场）。

### 15.4.1 run 与 enterWith：作用域语义的两面
...
前几节回答了"有什么"（族谱、观测窗、ALS）；这一节回答"它如何发生作用"。把暗线上的前三次穿越摆在一起看——第 3 章封装时埋下护照、第 4 章瓣膜放行时验照、第 6 章进程接力时另带——每一次上下文生效，都是同一套机制的三幕。

### 第一幕·捕获（创建时）

异步资源诞生那一刻——定时器入堆、`readFile` 投进线程池队列、Promise 挂上链——运行时把"当前正在执行的上下文"快照一份，钉在资源上。族谱实现里，这颗钉是 trigger 边；AsyncContextFrame 实现里，是帧的引用。从这一刻起，创建者去向何处、栈退成什么样，都与这份快照无关。

### 第二幕·休眠（等待期）

资源躺在最小堆里、epoll 的监控清单里、线程池的队列里——上下文随它一起休眠：不占栈、不消耗，静静等待瓣膜放行。这就是 15.1 节"存到比栈活得久的地方"的具象：**上下文不住在栈上，住在资源上**。

### 第三幕·换入换出（执行时）

回调进场前，运行时用资源里休眠的上下文换掉当前现场；回调返回后，再换回来。第 3 章 MakeCallback 三段式里的 CallbackScope，干的就是这次切换：
...
所谓"上下文传播"，从来不是数据在流动，而是**执行现场在切换**——捕获于创建，休眠于等待，恢复于执行。

### 断点清单：护照失效的三种场景

三幕反过来标定了失效点：**凡是没有完成"捕获于创建"的，执行时必然无照可验**。

1. **同步分发没有穿越。** 监听器在 `run` 内注册、却在 `run` 外被 `emit`——emit 是同步 for 循环（第 9 章），监听器看到的是 emit 现场的上下文，与注册现场无关。同步调用不经过三幕，自然无照。
2. **池化资源的护照签发日不对。** 连接池、任务队列的资源在启动时就创建完毕——捕获发生在"池诞生时"，不是"本次请求进来时"。回调执行时换入的是池的上下文，请求的 store 自然缺席。这正是 15.3.1 节 AsyncResource 存在的工程理由：给池里每个任务**补签一张当时的护照**。
3. **族谱之外的第三方原语。** 某些库用自有 C++ addon 或自定义调度分发回调，不经 AsyncWrap——族谱上根本没有这条边，链断在库的边界。修补手段同样是 AsyncResource：手动把它缝回族谱。

一句话：**断点皆源于"创建时没照上"**——要么同步调用压根没创建资源，要么资源创建得太早，要么创建时绕开了签发机关。

## 15.6 边界之外：上下文如何跨线程与进程

现在回到本章问题里最硬的部分。第 11 章说过：容器边界就是隔离边界。第 14 章说过：跨 Worker 只能传消息，跨进程只能走 IPC。那么——**ALS 的 store 能自己穿过去吗？**
...
这不是缺陷，而是第 11 章边界表的直接推论——"什么能共享、什么不能"由容器层级决定。要跨边界，就按第 14 章的规矩来：**把上下文当作普通数据，随消息显式携带，对面重新 run**：
...
| Worker 线程（第 14 章） | ❌ | postMessage / workerData 携带 + 对侧 run |
| 子进程 / Cluster（第 14 章） | ❌ | env / IPC 携带 + 对侧 run |
| 跨机器 | ❌ | 协议头透传（traceparent）+ 对端 run |

规律一目了然：**每穿过一道容器边界，就需要一次"显式携带 + 重新 run"**。边界的清单在第 11 章，穿墙的手法在本章——两章合起来，才是"上下文串联"的完整答案。

## 15.7 多条因果链并存：上下文的多实例正交
...
3. **机制三幕**——捕获于创建、休眠于等待（上下文住在资源上，不住在栈上）、换入换出于执行（CallbackScope 切换现场）；传播不是数据流动，是执行现场切换。
4. **AsyncLocalStorage 是应用层标准答案**——run 划域、getStore 取值；run(async fn) 返回 Promise；enterWith 仅用于"从此处起"的长会话场景。
5. **两代实现**：族谱回溯 → AsyncContextFrame 帧随延续体传播；新实现默认启用，生产可用。
6. **多实例正交、同实例遮蔽**——不同维度的上下文走不同 ALS 实例；同实例嵌套 run 遮蔽外层是作用域语义的自然结果，而非特殊行为。
7. **断点皆源于"创建时没照上"**——同步分发无穿越、池化资源签发日过早、第三方原语绕开签发机关；统一用 AsyncResource 补签。
8. **传播半径 = Environment 边界**——跨 Worker / 子进程 / 机器一律"显式携带 + 重新 run"；每道容器边界一次。
9. **store 最小化**——只放标识符与元数据；大对象走参数或按 ID 查，别塞进 store。

## 下一章引子

十五章，零件与容器、因果与边界，全部就位。是时候兑现前言的承诺了——
...
[← 上一章：多核之路](./ch14-multicore.md) | [终章：万物归一 →](../ch16-epilogue.md)
